...
- [ ] **Stage 2 — graduate a trusted type to autonomous.** The auto-runner that consults `canAutoExecute` and flips `agent_policy.auto_execute=true` for `create_org_row` once its approval rate holds ~100% for `min_clean_weeks`. Outward types never graduate (the guardrail forbids it). Also: retrofit the newsletter send as the highest-value Stage 2 safety win.
- [ ] **Web-research auto-fill for `create_org_row`.** The proposer currently leaves `canonical_url` blank for the human; add a Sonnet + `web_search` step (mirror `submission-triage.ts`) to pre-fill the organizer's events URL, so approving is a true one-click.
