...
- **`validateEventUrls`** ([scripts/lib/validate-urls.ts](scripts/lib/validate-urls.ts)) skips `UNSTABLE_SOURCE_HOSTS` (it can't see them and we don't render them — a 403 there is not a dead link). **`/api/check-events`** reports `aggregator_link_gaps`: upcoming events still showing no outbound link because they're aggregator-sourced with no organizer canonical yet. Each is closeable with one `hwy4_orgs` row. Watch it trend down.
