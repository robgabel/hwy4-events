...
- **PRDs & Plans:**
  ...
  - `PRD-agent-cockpit.md` — A supervised "business-manager" layer over the existing automation. The data layer is ~75–80% zero-person; the *decision* layer (triaging Slack alerts, the verification queue, submissions, `hwy4_orgs` rows, gate flips) is 100% manual and scattered. Plan: three tiers (dumb **collectors** write rows → one **reasoner** agent proposes actions on DB state → a **cockpit** under `/admin` to approve them, reusing Basic Auth `middleware.ts` + the `verification/actions.ts` server-action pattern). Staged on the codebase's own dry-run → canary → reversible arc: Stage 0 read-only digest, Stage 1 propose-human-approves, Stage 2 graduate low-stakes *internal* reversible actions to autonomous behind per-type `agent_policy` flags (the per-action `RECONCILE_EXECUTE`). Outward/editorial actions never auto-run; every executed action carries a reversible `before_snapshot` (mirrors `event_merge_log`). Also: **buy** the FB-group sensor (Devi AI/GroupsWatcher — the Groups API died 2024-04-22; reply stays manual), **build** the SEO/AEO collector (GSC + Bing APIs), and retrofit a human gate onto the one unsupervised outward action (the Thursday newsletter auto-send). **Stage 0 built 2026-05-31** (report-only, executes nothing): migration `20260531c_agent_cockpit_stage0.sql` (`agent_runs` + `seo_snapshots`, RLS service-role only); `/api/agent/chief-of-staff` (Sonnet reasoner) + `/api/agent/collect-seo` (zero-dep GSC client `lib/agent/gsc.ts`); `/admin/today` digest page in the existing Basic-Auth admin area; digest shape in `lib/agent/types.ts`; crons at 11:00 + 19:00 UTC. **To go live:** apply the migration, set `GOOGLE_SEARCH_CONSOLE_SA_JSON`, deploy. Stages 1-2 (action queue, `agent_policy` graduation, FB/AEO collectors, newsletter gate) pending.
  - `PRD-event-poster-loop.md` — Organizer-led viral growth loop: turn every event detail page into a beautiful, branded, shareable poster (web hero + OG image + downloadable PDF) organizers reach for instead of a Facebook-only JPEG. Thesis: *the page is the poster; the poster is the billboard.* One artifact serves two organizer types (digital reach for poster-havers; a free downloadable PDF poster for have-nots); Millie + `hwy4events.com` lockup + a QR (encoding `?src=qr&e=<id>`) turn every share and every printed flyer into attributed marketing, closing the loop through both a digital and a physical door. Poster system: AI **art-directs a single templated design** (8 category skins, type-first, local motifs) — never freeform-illustrates, because consistency = brand recognition. Non-goals: no RSVP/ticketing, not a general listings site, not Canva, no bespoke per-event AI art. Attribution event-level now (`ShareButton` + a `share_hits` table), org-level (`?via=`) after claim; claim/upload reuses `event_submissions` + `/admin`; season-poster→many-rows reuses `isGenericTitle`/umbrella-series; render reuses `app/og/`. Staged: validate the loop on poster-having events first, then build the generator behind a human taste gate. **Phase 1 + generator implemented 2026-06-03** (worktree; production build passes, poster renders live from the DB): `app/events/[slug]/poster/route.tsx` (next/og + Satori screenprint, Node runtime, fonts from `public/fonts/`), detail page rebuilt around the poster hero + action rail, supplied posters (`image_url`) shown untouched, `share_hits` attribution (`20260603_add_share_hits.sql` + `/api/track-share` + `components/ShareTracker.tsx` + `ShareButton ?src=share`), shared `lib/poster.ts` + `lib/events.ts`. **To deploy:** apply `20260603_add_share_hits.sql` to Supabase + merge → Vercel. Follow-ups: real QR encoder, full 8-category motif set. Originated in a `/brain scott-belsky` session.
