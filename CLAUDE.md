...
| `/api/agent/chief-of-staff` | Daily 7pm UTC (after the 6pm audit) | **Agent Cockpit Stage 0 reasoner.** Reads the verification queue, pending `event_submissions`, 24h auto-merges, and the latest `seo_snapshots`, asks Sonnet (`claude-sonnet-4-6`) to triage them into a short digest, writes one `agent_runs` row, and posts a one-liner to Slack linking `/admin/today`. **Read-only: proposes/executes nothing.** See `PRD-agent-cockpit.md`. |
