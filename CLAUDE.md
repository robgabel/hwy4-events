...
| `/api/newsletter/send` | Thursdays 3pm UTC | Ship the week's newsletter on a **24h veto window**: auto-sends today's `newsletter_drafts` row *unless* a human vetoed it at `/admin/newsletter` (the ~24h between Wed prepare and Thu send). No draft at all (prepare didn't run) → sends nothing and Slack-warns; it never auto-generates-and-blasts unsupervised. **Delivery uses Resend's Batch API** (`resend.batch.send`, ≤100 messages/request) so the whole list ships in one HTTP call and can't trip Resend's 5 req/sec rate limit, and it inspects each `{ error }` so `sent_count` reflects only messages Resend actually accepted. *(Fixes the 2026-06-04 incident: the old per-recipient `emails.send` loop ignored the SDK's non-throwing `{ error }` return, so Resend 429-rate-limited 42 of 72 sends while the code counted all 72 as "sent". Each message still carries its own personalized unsubscribe link + `List-Unsubscribe` header; a whole-batch rejection falls back to throttled per-message sends.)* On send, marks the draft `sent` and archives the body to `site_config.latest_newsletter`. **Slack-alerts if `sent < total`** so a partial send is never silent. `?preview=1` renders the current draft (no auth, no send); `?test_email=…` generates fresh + sends to one address (template smoke-test, bypasses the gate). |
...
- **`/api/check-events` link-gap KPI.** The audit reports two things: `aggregator_fallback_links` (raw count of events whose best link is only the non-durable GoCalaveras fallback — trend signal) and `actionable_link_gaps` — distinct venues with **≥5 upcoming events** using only the fallback (`GAP_VENUE_THRESHOLD` in the route). The Slack post lists those venues by name and fires on a high-frequency gap even with zero other issues — so a new recurring organizer (a new concert series, a new winery) surfaces itself for a ~10-min `hwy4_orgs` row. Watch `actionable_link_gaps`, not the raw count.
