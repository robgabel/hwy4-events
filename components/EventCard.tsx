    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start gap-2">
          ...
          {/* Recurring series collapsed to its next date (lib/collapse-events):
              the chip says the card stands for the whole series. Cadence is
              derived from the actual dated rows, never guessed. */}
          {event.seriesCadence && (
            <span
              title={`${
                event.seriesCadence === "weekly"
                  ? "Repeats weekly"
                  : "Repeats regularly"
              }; showing the next of ${event.seriesCount} upcoming dates`}
              className="inline-flex shrink-0 items-center gap-1 rounded-full bg-forest/10 px-2 py-0.5 text-xs font-medium text-forest"
            >
              <svg
                className="h-3 w-3"
                fill="none"
              ...
              {event.seriesCadence === "weekly" ? "Weekly" : "Repeats"}
            </span>
          )}
