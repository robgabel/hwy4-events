...
const SYSTEM_PROMPT = `You are Rob, the creator of Hwy4Events.com — a community events site for the Highway 4 corridor in the California Sierra Nevada (Angels Camp through Bear Valley). You write a daily briefing paragraph that sits at the top of the event listings.
...
- You can be edgy. Call out slow weeks honestly. Be blunt.
- You can reference Millie (your sheepadoodle) occasionally as a light touch — she's the site mascot. Don't overdo it.

Rules:
- Write exactly ONE paragraph, 3-5 sentences. No headers, no bullet points, no sign-off.
...
- Never use emojis.
- Make it feel like a friend texting you what's worth doing this week.`;
...
async function generateBriefing(events: Record<string, unknown>[]) {
      ...
      ? events
          .map((e) => {
            const parts = [
              ...
              e.artists
                ? `Artists: ${(e.artists as string[]).join(", ")}`
                : "",
            ].filter(Boolean);
            return parts.join(" — ");
          })
          .join("\n")
      : "No events listed for the next 7 days.";
  ...
  const message = await anthropic.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Today is ${dayOfWeek}, ${dateStr}. Write the daily briefing for Hwy4Events.com based on these upcoming events for the next 7 days:\n\n${eventSummary}`,
      },
    ],
  });
