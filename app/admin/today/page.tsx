...
export default async function TodayPage() {
  ...
  return (
    <div style={{ maxWidth: 940, margin: "0 auto" }}>
      <h1 style={{ color: "#2d5016", fontSize: 24, margin: "0 0 4px" }}>Today</h1>
          ...
          {digest && (
            <>
              <section style={summaryCardStyle}>
                <p style={{ margin: 0, color: "#2d3a22", fontSize: 16, lineHeight: 1.6 }}>
                  {digest.summary}
                </p>
              </section>
    ...
    >
      {stats.map((s) => (
        ...
        >
          ...
          <p style={{ color: "#2d5016", fontSize: 22, fontWeight: 700, margin: 0 }}>{s.value}</p>
        </div>
      ))}
    </div>
  );
...
function ItemCard({ item, variant }: { item: DigestItem; variant: "needs" | "fyi" | "watching" }) {
  ...
  return (
    ...
    >
      <h3 style={{ color: "#2d5016", fontSize: 16, margin: "0 0 6px", fontWeight: 600 }}>
        {item.title}
      </h3>
      <p style={{ color: "#3a3a3a", fontSize: 14, lineHeight: 1.55, margin: 0 }}>{item.detail}</p>
      {item.why && (
        <div
          style={{
            background: "#fff7ed",
        ...
        >
          ...
          <p style={{ color: "#3a3a3a", fontSize: 13, lineHeight: 1.5, margin: 0 }}>{item.why}</p>
        </div>
      )}
      {item.link && (
...
function Banner({
  ...
  return (
    <section style={{ ...styles, borderRadius: 12, padding: "14px 18px" }}>
      <p style={{ margin: "0 0 2px", fontSize: 15, fontWeight: 600 }}>{title}</p>
      <p style={{ margin: 0, fontSize: 13, lineHeight: 1.5, opacity: 0.9 }}>{children}</p>
    </section>
  );
}
