...
- **Privacy: store the class, never the IP.** For a row tied to an email (PII), store only `visitor_class` (local/hub/visitor/unknown), not city/region — don't attach a location to a named person. (Anonymous `site_events` rows can hold coarse geo; named subscriber rows shouldn't.)
- **Check an IP classifier against your own connection before trusting its split.** The Gate 0 read-out ran for three months saying ~3% local. Rob, physically in Arnold on Comcast, geolocates to Lodi; Valley Springs (a Calaveras County town, 299 sessions) sat just outside the box and read "visitor". A regional ISP hub city is neither local nor visitor and nothing in the IP splits it, so it got its own class (`hub`, 2026-09-04) rather than a guess either way. The local share is a floor, never a fact; and the hub list is region config, because the Eugene fork has different hubs.
...
## 2026-08-19 — Program wrap: the red-run fix and what three adversarial reviews taught

### Data pipeline / normalization

- **A normalize step that creates the condition another step needed must re-run that step.** The address-in-venue-name swap (`normalizeEventLocation`) produced exactly the input the venue matcher's address layer wants — but the per-scraper detection pass had already run, so nothing ever looked again. The row ("Arnold Angels Music Festival": EventON put Brice Station's street address in the location *name* field, town guessed off the charity's name) was structurally unable to self-heal: every re-scrape replayed the identical sequence and compared equal, failing the location sanity check daily. The machinery was all there, just ordered around itself. Fix shape: re-run detection *inside* the swap, in the shared path, so the whole class self-heals.
- **When re-running a resolver in a new context, gate it to the evidence class that justified the re-run.** The first cut called the full matcher; with `venue_name` now generic, its TEXT layer would adopt a registry venue merely *mentioned* in the title/description/URL — moving `town` off a mention, contradicting the row's own address (manufacturing the very `TOWN_CONFLICT` being fixed), changing the town-bearing event URL with no fallback, and silencing `UNRESOLVED_VENUE`. The swap's justification is the address, so only an address-anchored match (`matched_alias` `address:`/`address-prefix:`) earns the adoption. A mention is not an address.
- **Verify a "the column is empty" premise at DROP time, not at plan time.** The T5 audit said `importance*` all-NULL and `sources` all-`[]`; at drop time prod held 26 importance values and 579 legacy sources arrays. The ratified archive-first branch (`hwy4_events_dropped_cols_archive`, select-into before `DROP COLUMN`) made the stale premise harmless. Two-phase drops should always carry that branch.

### Sensors / ops

- **"A sensor that can fail silently is not a sensor" — third instance: the ALERT path itself.** The scrape workflow's failure step files a GitHub issue, but the default `GITHUB_TOKEN` is read-only, so the alert 403'd ("Resource not accessible by integration") and two red days went unannounced. After `scrape_runs` inserts (instance 1) and the sub-floor cap logging (instance 2), the family rule is now: when adding any failure alert, prove the alert itself can fire (permissions, env, network) — trigger it once on purpose if you can.
- **In a remote autonomous session, expect prod data mutations and repo *settings* to be blocked, and design the handoff as a one-click ask.** The permission layer denied event-row DELETE/UPDATE and workflow-file arming edits; the GitHub integration cannot write repo variables or dispatch workflows. The pattern that worked: prepare the exact reversible SQL / the exact Settings path + value, hand it to Rob as one click, verify the result after. Don't extract credentials or route around the boundary — the boundary is the design.
- **Before treating a blocker ticket as open, check the board AND the code.** HWY-26 (aggregator sole-witness gap) was filed as the gocalaveras-graduation blocker; by the review date a parallel session had built and shipped it, and the "34 rows excluded (HWY-26)" lines were already live in every sweep log. Ten seconds of `select status` and one log grep beat re-litigating a solved problem.

### Process

- **The adversarial-review gate caught a real, shippable-looking defect in three consecutive PRs** — masked cap-candidates in #253, the TBD-retraction/name-steal over-breadth in #264 (which would have permanently advertised cancelled acts, silently), and the text-layer town adoption in #265 (which would have manufactured the exact red it fixed). All three passed 600+ tests and tsc. For matcher/deletion-adjacent changes, the refute-oriented reviewer with DB access is load-bearing, not ceremony; its cost (~10 min/PR) bought three averted prod incidents in one week.

- **A proxy metric substituted for a specified log check must be proven equivalent — especially when the proxy runs AFTER the healer.** The read-time assertion's removal gate said "watch the logs for `READTIME_DEDUPE_ASSERT`; silence means the layers hold." Unable to read Vercel logs, the removal PR substituted daily audit zeros — but the audit (18:00 UTC) runs after reconcile (15:30), so any duplicate reconcile heals is structurally invisible to it, and `event_merge_log` proved two duplicates DID transit the render path inside the "clean week." The adversarial reviewer caught the inference before the false narrative landed in Tier-2 docs. The honest gate that justified the same merge: zero PERSISTENT duplicates + transits that self-heal within hours in the lowest-traffic window, with `merges_last_24h` as the transit signal. State the accepted trade; never claim silence you didn't hear. Corollary: know your metric's filters — the cited `same_event_duplicates` additionally drops same-NAME clusters, which both transits were.

---

## 2026-09-05 — The health report: two faults that cancelled into silence

### Sensors / ops

- **"A sensor that can fail silently is not a sensor" — fourth instance: the source list itself.** The nightly scrape health report derived its list of sources by selecting the whole `org_slug` column and taking the distinct values. PostgREST caps an unbounded select at 1,000 rows and `hwy4_events` had grown to 1,844, so the report saw exactly the 13 slugs that fit in the first page and every source sorting after `gocalaveras` was invisible — including `visit-murphys`, the corridor's biggest feed at 108 upcoming rows. The report whose entire job is to notice a silently-failing scraper could not have noticed six of them fail, and was already hiding two live findings (`hinterhaus-distilling` and `murphys-library` had both drained to zero upcoming events). After `scrape_runs` inserts, the sub-floor cap logging, and the GitHub alert token, the family rule grows a clause: **a sensor whose scope is derived from a query has a scope you must prove, not assume.** Any distinct-values-from-a-column read is a truncation waiting for the table to grow past the cap.
- **Page by what came back, not by what you asked for.** The obvious fix — request 1,000 rows and stop when a page returns fewer — silently rebuilds the same bug if the server's own row cap is lower than the window requested: every page is short, the first one ends the loop, and the truncation is back with a paging loop wrapped around it. Advance the offset by `rows.length` and stop only on zero.
- **A report that cries wolf every morning is how a real failure hides.** All 11 of that night's warnings were false positives, and had been for months, which is precisely why nobody was reading the report closely enough to notice it was also missing ten sources. The two faults were not independent: the noise was the camouflage.
- **One staleness rule cannot describe sources with different writers.** "No row written in >7 days ⇒ the scraper may be silently failing" is true only for sources the nightly Action runs. A hand-transcribed **seed** source passes 7 days the week after every seeding and never comes back. An **external-cron** source written by a Vercel route that stamps `last_scraped_at` only on INSERT measures how long since the venue posted, not whether the route ran — and this Action cannot see that route at all. A **retired** source has no writer. Encode the ownership as data with the writer named, default the unknown case to the loud option, and let the map be the decision record.
- **"Days since we last wrote a row" is a property of the source's posting cadence, not of our health.** Bear Valley Mountain Resort's events page held nothing but two lodging promos for eleven days; the scraper read it correctly and extracted nothing, which the report called "may be silently failing." What is ours to catch is a source draining to zero, or a scraper vanishing from the dispatch table. What a source running clean while adding nothing means over time is a 14-day question, and the weekly scraper-health memo already owns it.
- **An identity mapping is where drift hides.** `scrapedSources.includes(org_slug)` looked obviously correct because dispatch key and `org_slug` match for twelve of fourteen scrapers. They differ for two, and `fb-discover-arnold` had therefore read as "no scraper ran for it" every single night. Make the mapping explicit and pin it to the real dispatch table in a test; the pin is cheap and the assumption is invisible.
